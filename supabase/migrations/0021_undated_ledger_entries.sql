-- ============================================================
-- 0021 — Undated ledger entries shouldn't borrow "now" as their date
--   Imported rows with no date ("Multiple Dates", e.g. early club setup) were
--   falling back to granted_at (the import time), so they showed today's date and
--   wrongly counted toward "this term". Treat IMPORT rows by their entry_date only
--   (null = genuinely undated → shown as undated, excluded from term). Role grants
--   and other non-import ledger rows still fall back to granted_at as before.
-- ============================================================
